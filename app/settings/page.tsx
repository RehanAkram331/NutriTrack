...
function DownloadAppBanner() {
  ...
  useEffect(() => {
    ...
    const isInApp = ua.includes('CalorieCanvasApp')
    setShow(isAndroid && !isInApp)
  }, [])
    ...
    <div className="mt-5 rounded-2xl overflow-hidden border border-slate-800"
      ...
      <div className="p-5 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-slate-100 text-[14px]">Get the Android App</div>
          <div className="text-slate-500 text-[12px] mt-0.5">Install for a native experience</div>
        </div>
        <a
          href="/caloriecanvas.apk"
          download="CalorieCanvas.apk"
          className="flex items-center gap-1.5 px-4 py-2 rounded-[8px] font-semibold text-[12px] text-white no-underline transition-all hover:opacity-90 active:scale-95 whitespace-nowrap flex-shrink-0"
          style={{ background: 'linear-gradient(135deg,#22d3ee,#818cf8)' }}
        >
          ⬇ Download
        </a>
